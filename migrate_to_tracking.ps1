...
if ([int]$functionCount -ge 6) {
    ...
    Write-Host "⚠️ WARNING: Some ETL functions may be missing (only $functionCount functions found)" -ForegroundColor Yellow
...
"@
    ...
    Write-Host "ℹ️ Basic ETL functions created. For full functionality, ensure database/03_etl_procedures.sql is properly configured." -ForegroundColor Yellow
}
...
Write-Host "🔍 Testing demo website..." -ForegroundColor Yellow
try {
    $siteResponse = Invoke-WebRequest -Uri "http://localhost/" -UseBasicParsing
    if ($siteResponse.Content -match "Analytics Test Site") {
        Write-Host "✅ SUCCESS: Demo website is accessible" -ForegroundColor Green
    } else {
        Write-Host "❌ ERROR: Demo website content invalid" -ForegroundColor Red
        exit 1
    }
} catch {
    Write-Host "❌ ERROR: Demo website is not accessible" -ForegroundColor Red
    exit 1
}
...
Write-Host "   • Demo Site:   http://localhost" -ForegroundColor White
...
Write-Host "   • Tracking: tracking/README.md" -ForegroundColor White
Write-Host ""